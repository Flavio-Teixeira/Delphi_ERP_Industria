...
type
  TVHistoricoProdutos00 = class(TForm)
    Panel2: TPanel;
...
var
  VHistoricoProdutos00: TVHistoricoProdutos00;

implementation

uses Conexao_BD, Rotinas_Gerais, Ibrasmak_00, V_Historico_Produtos_03;       

var Selecionado: string;

{$R *.dfm}

procedure TVHistoricoProdutos00.BT_ProcurarClick(Sender: TObject);
begin
     Ampulheta();
...
procedure TVHistoricoProdutos00.Dados_ProcuraKeyPress(Sender: TObject; var Key: Char);
begin
     If Key = #13 Then
        Begin
        Key := #0;
...
procedure TVHistoricoProdutos00.Opcoes_ProcuraKeyPress(Sender: TObject; var Key: Char);
begin
     If Key = #13 Then
        Begin
        Key := #0;
...
procedure TVHistoricoProdutos00.DBGrid_ProdutosCellClick(Column: TColumn);
var Codigo_Produto: String;
begin
    If DBGrid_Produtos.Fields[0].Text <> '' Then Begin
      Selecionado := DBGrid_Produtos.Fields[0].Text;

      VHistoricoProdutos00.Visible := False;

      Ampulheta();
      Application.CreateForm(TVHistoricoProdutos03,VHistoricoProdutos03);

      VHistoricoProdutos03.Tipo_Historico.Text := Tipo_Historico.Text;
      ...
      VHistoricoProdutos03.editReferencia.Text := ConexaoBD.SQL_Produtosfb_produto_referencia.Text;
      VHistoricoProdutos03.editCodigo.Text     := ConexaoBD.SQL_Produtosfb_produto_codigo.Text;
      VHistoricoProdutos03.editDescricao.Text  := ConexaoBD.SQL_Produtosfb_produto_descricao.Text;
      ...
      VHistoricoProdutos03.ShowModal;
      VHistoricoProdutos03.Destroy;

      VHistoricoProdutos00.Visible := True;     

      ConexaoBD.SQL_Produtos.Locate('fb_produto_codigo',Trim(Selecionado),[]);
    End;
end;

procedure TVHistoricoProdutos00.FormCreate(Sender: TObject);
begin
...
procedure TVHistoricoProdutos00.FormShow(Sender: TObject);
begin
    ConexaoBD.SQL_Produtos.SQL.Clear;
    ConexaoBD.SQL_Produtos.SQL.Add('Select * from fb_produtos Order By fb_produto_codigo');
    ConexaoBD.SQL_Produtos.Open;
...
procedure TVHistoricoProdutos00.BT_SairClick(Sender: TObject);
begin
    VHistoricoProdutos00.Close;
end;
