unit Notas_Fiscais_Entradas_00;
...
type
  TNotasFiscaisEntradas00 = class(TForm)
    Panel2: TPanel;
...
var
  NotasFiscaisEntradas00: TNotasFiscaisEntradas00;
  Selecionado: string;

implementation

uses Conexao_BD, Rotinas_Gerais, Notas_Fiscais_Entradas_01,
  Notas_Fiscais_Entradas_02, Notas_Fiscais_Entradas_06, Rel_Faturamento_01;

{$R *.dfm}

procedure TNotasFiscaisEntradas00.BT_SairClick(Sender: TObject);
begin
     NotasFiscaisEntradas00.Close;
end;

procedure TNotasFiscaisEntradas00.Dados_ProcuraKeyPress(Sender: TObject;
  var Key: Char);
begin
     If Key = #13 Then
        Begin
...
procedure TNotasFiscaisEntradas00.Opcoes_ProcuraKeyPress(Sender: TObject;
  var Key: Char);
begin
     If Key = #13 Then
        Begin
...
procedure TNotasFiscaisEntradas00.BT_ProcurarClick(Sender: TObject);
begin
     Ampulheta();
...
procedure TNotasFiscaisEntradas00.FormCreate(Sender: TObject);
begin
...
procedure TNotasFiscaisEntradas00.FormShow(Sender: TObject);
begin
...
procedure TNotasFiscaisEntradas00.BT_IncluirClick(Sender: TObject);
begin
...
procedure TNotasFiscaisEntradas00.DBGrid_NotasCellClick(Column: TColumn);
var
    Posicao_Nota: String;
    Erro: Integer;
    Vlr_Numero: Real;
begin
    If DBGrid_Notas.Fields[0].Text <> '' Then Begin
      ...
      If (NotasFiscaisEntradas02 = Nil) Then
        ...
        Posicao_Nota                             := DBGrid_Notas.Fields[4].Text;
...
procedure TNotasFiscaisEntradas00.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
     Action := caFree;
     Release;
     NotasFiscaisEntradas00 := Nil;
end;

procedure TNotasFiscaisEntradas00.BT_ImprimirClick(Sender: TObject);
...
{    Ampulheta();
    ...
    Application.CreateForm(TNotasFiscaisEntradas06,NotasFiscaisEntradas06);

    NotasFiscaisEntradas06.QR_Periodo.Caption := 'Período de ' + Trim(Data_Inicial.Text) + ' a ' + Trim(Data_Final.Text);

    Seta();

    NotasFiscaisEntradas06.QR_ProdutosComprados.Preview;
    NotasFiscaisEntradas06.Destroy; }
